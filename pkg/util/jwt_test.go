...
func (mc *mockHttpClient) Do(_ *http.Request) (*http.Response, error) {
	pwd, _ := os.Getwd()
	var fn = pwd + "/testdata/jwt_test_data.firebase.json"
	println(fn)
	if mockClientSwitch {
		fn = pwd + "/testdata/jwt_test_data.google.json"
	}
...
func TestJWTUtils(t *testing.T) {
	...
	t.Run("Test reading Google JWK json", func(t *testing.T) {
		decoder := NewJWTDecoder(testdatadir + "/jwt_test_data.google.json")
		assert.True(t, decoder.CheckReady())
	})

	t.Run("Test reading Google firebase Key set", func(t *testing.T) {
		decoder := NewJWTDecoder(testdatadir + "/jwt_test_data.firebase.json")
		assert.True(t, decoder.CheckReady())
	})
	...
	t.Run("Test Cloudflare Access Support", func(t *testing.T) {
		...
		cloudflareJwkPath := testdatadir + "/jwt_test_data.cloudflareaccess.json"
